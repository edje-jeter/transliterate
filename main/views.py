...
def json_response(request):
    ...
    print search_string
    objects = DictCMU.objects.filter(entry__iexact=search_string)
...
def input_output(request):
    ...
    if request.method == 'POST':
        ...
        if form1.is_valid():
            ...
            # Map the punctuation, capitalization, and words.
            # At this point I'm not dealing with the possibility of capitals
            # within words---only at the beginning of words and/or sentences.
            space_count = english_input_v.count(' ')
            context['spaces'] = space_count
            ...
            for inp in input_map2:
                # print unic_out
                # print "inp: %s" % inp
                # if word_list[ticker] == "NO MATCH":
                #     unic_out.append("NO MATCH")
                #     ticker += 1
                if inp in ("CC", "C", "Cm", "mm", "m"):
                    ...
                    if arpa_phonemes[word_list[ticker]] == "NO MATCH":
                        ...
                        for phone in arpa_phonemes[word_list[ticker]]:
                            # print "phone: %s" % phone
                            # print "arpa_corr_U: %s" % arpa_correlation_U[phone]
                            # a = arpa_correlation_U[phone]
                            # print "a: %s" % a
                            # b = a.encode("utf-8")
                            # print "b: %s" % b
...
# unic_out = []
# for value in arpa_phonemes.values():
#     for val in value:
#         unic_out.append(arpa_correlation_U[val])
# print unic_out
